	 ...
	 CASE WHEN net_sales/8 >= 250000 THEN '01) A1'
          ...
          CASE WHEN c.cust_assort_section_id IS IN (1) THEN '1) HoReCa'
               ...
               ELSE '4) Undefined' end as branch,
                   CASE WHEN d.wel_ind = 'Y' THEN 'Wel'
                        WHEN d.fsd_ind = 'Y' THEN 'Fsd'
                        WHEN d.deli_ind = 'Y'  THEN 'O2O dealers'
                        WHEN d.bvs_ind = 'Y'  THEN 'O2O dealers'
                        WHEN d.pcr_ind = 'Y'  THEN 'O2O dealers'
                        ELSE 'O2O FoM' END AS VCP,
                        ...
                        group by t.home_store_id, t.cust_no, t.auth_person_id, branch, VCP
	...
	as(select a.home_store_id, a.cust_no, a.auth_person_id,
		CASE WHEN net_sales/8 >= 40000 THEN '01) A'
	         ...
	         CASE WHEN net_sales/8 >= 250000 THEN '01) A1'
                  ...
                  group by a.home_store_id, a.home_store_id, a.auth_person_id
                  having net_sales >0
                  )with data;
...
create table chnccp_msi_z.LP_sales_and_redemption_1
	as(select a.*, b.pts_collected, b.pts_used, b.month_points, b.advance_points, b.flash_points, b.KA_points from chnccp_msi_z.LP_namelist_total_1 a 
		left join chnccp_msi_z.LP_namelist_points_total b
		on a.home_store_id = b.home_store_id and a.cust_no = b.cust_no and a.auth_person_id = b.auth_person_id
		)with data;
...
sum(month_points) as month_points, sum(advance_points) as advance_points, sum(flash_points) as flash_points, sum(KA_points) as KA_points from chnccp_msi_z.LP_sales_and_redemption_1
group by ABC
order by ABC;

select ABC_combined, sum(net_sales) as  sales, avg(visit) as visit, sum(front_margin) as front_margin, count(distinct home_store_id||cust_no||auth_person_id) as cust, 
sum(pts_collected) as collected_points, count(distinct CASE WHEN pts_collected >0 THEN home_store_id||cust_no||auth_person_id ELSE NULL END) as collected_cust,
sum(month_points)+sum(advance_points)+sum(flash_points)+sum(KA_points) as pts_used, count(distinct CASE WHEN pts_used >0 THEN home_store_id||cust_no||auth_person_id ELSE NULL END) as used_cust,
sum(month_points) as month_points, sum(advance_points) as advance_points, sum(flash_points) as flash_points, sum(KA_points) as KA_points from chnccp_msi_z.LP_sales_and_redemption_1
...
select branch, sum(net_sales) as  sales, avg(visit) as visit, sum(front_margin) as front_margin, count(distinct home_store_id||cust_no||auth_person_id) as cust, 
sum(pts_collected) as collected_points, count(distinct CASE WHEN pts_collected >0 THEN home_store_id||cust_no||auth_person_id ELSE NULL END) as collected_cust,
sum(month_points)+sum(advance_points)+sum(flash_points)+sum(KA_points) as pts_used, count(distinct CASE WHEN pts_used >0 THEN home_store_id||cust_no||auth_person_id ELSE NULL END) as used_cust,
sum(month_points) as month_points, sum(advance_points) as advance_points, sum(flash_points) as flash_points, sum(KA_points) as KA_points from chnccp_msi_z.LP_sales_and_redemption_1
group by branch
...
--other redemption
drop table chnccp_msi_z.LP_namelist_points_other_PY;
create table chnccp_msi_z.LP_namelist_points_other_PY
	as(select tt.home_store_id, tt.cust_no, tt.auth_person_id, sum(tt.advance_points*a.redemption_points) as advance_points from ((select b.home_store_id, b.cust_no, b.auth_person_id, a.mikg_art_no, sum(b.sell_qty_colli) as advance_points from chnccp_msi_z.redemption a 
		join chnccp_dwh.dw_art_var_tu c  
		on a.mikg_art_no = c.mikg_art_no
		join chnccp_fls.view_frank_auth_person_invoice_line_channel_adj b
...
create table chnccp_msi_z.LP_namelist_points_total_PY
	as(select a.*, b.month_points, c.advance_points, d.advance_points as other_points from chnccp_msi_z.LP_namelist_points_PY a 
		left join chnccp_msi_z.LP_namelist_points_monthly_PY b 
		on a.home_store_id = b.home_store_id and a.cust_no = b.cust_no and a.auth_person_id = b.auth_person_id
		left join chnccp_msi_z.LP_namelist_points_advance_PY c
		on a.home_store_id = c.home_store_id and a.cust_no = c.cust_no and a.auth_person_id = c.auth_person_id
		left join chnccp_msi_z.LP_namelist_points_other_PY d 
		on a.home_store_id = d.home_store_id and a.cust_no = d.cust_no and a.auth_person_id = d.auth_person_id)with data;
...
create table chnccp_msi_z.LP_sales_and_redemption_PY_1
	as(select a.*, b.pts_collected, b.pts_used, b.month_points, b.advance_points, b.other_points from chnccp_msi_z.LP_namelist_total_1_PY a 
...
select ABC, sum(net_sales) as  sales, avg(visit) as visit, sum(front_margin) as front_margin, count(distinct home_store_id||cust_no||auth_person_id) as cust, 
sum(pts_collected) as collected_points, count(distinct CASE WHEN pts_collected >0 THEN home_store_id||cust_no||auth_person_id ELSE NULL END) as collected_cust,
sum(month_points)+sum(advance_points)+sum(other_points) as pts_used, count(distinct CASE WHEN pts_used >0 THEN home_store_id||cust_no||auth_person_id ELSE NULL END) as used_cust,
sum(month_points) as month_points, sum(advance_points) as advance_points, sum(other_points) as other_points from chnccp_msi_z.LP_sales_and_redemption_PY_1
group by ABC
order by ABC;
...
select sum(a.pts_collected) as collected_points, count(distinct CASE WHEN pts_collected >0 THEN a.home_store_id||a.cust_no||a.auth_person_id ELSE NULL END) as collected_cust,
sum(a.month_points)+sum(a.advance_points)+sum(a.other_points) as pts_used, count(distinct CASE WHEN a.pts_used >0 THEN a.home_store_id||a.cust_no||a.auth_person_id ELSE NULL END) as used_cust,
sum(a.month_points) as month_points, sum(a.advance_points) as advance_points, sum(a.other_points) as other_points
